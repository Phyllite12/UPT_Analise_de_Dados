...
adaboost_pca = AdaBoostClassifier(base_estimator=tree.DecisionTreeClassifier(criterion='gini', max_depth=2),
                                  n_estimators=50,
                                  learning_rate=1.0)

# Treinar o modelo com dados transformados por PCA
adaboost_pca.fit(X_train_pca, y_train)

# Fazer previsões no conjunto de teste com dados transformados por PCA
y_pred_pca = adaboost_pca.predict(X_test_pca)

# Calcular a precisão do modelo com dados transformados por PCA
accuracy_pca = adaboost_pca.score(X_test_pca, y_test)
print("Precisão (com PCA):", accuracy_pca)
